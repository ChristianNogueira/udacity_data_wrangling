...
DATAFILE = 'C:\\Users\\christian\\Documents\\udacity\\data_wrangling\\data\\arachnid.csv'
            ...
            for key in line:
                for fld in FIELDS:
                    if fld == key:
                        if line[key] != 'NULL': 
                            if re.search('_', fld):
                                clas[FIELDS[fld]] = line[key]
                            else:
                                mask[FIELDS[fld]] = line[key]
                        else:
                            if re.search('_', fld):
                                clas[FIELDS[fld]] = None    
                            else:    
                                mask[FIELDS[fld]] = None    
            
            if mask['name'] == None:
                mask['name'] = mask['label']
            
            if mask['synonym'] != None:
                mask['synonym'] = parse_array(mask['synonym'])
                
            mask['label'] = re.sub(r'\s*\([^)]*\)\s*', '', mask['label'])
            mask['classification'] = clas
            data.append(mask)
